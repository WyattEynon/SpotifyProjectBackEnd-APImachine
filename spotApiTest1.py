...
results = sp.current_user_top_tracks(limit=TRACK_LIST_LIMIT,offset=0,time_range="long_term")

f = open("userfile1.csv", "w")
...
for idx, item in enumerate(results['items']):
    ...
    if(idx<TRACK_LIST_LIMIT):
        f.write(((str(idx))+","+item["name"]+","+albumDisplay+","+artistsDisplay+","+releaseDisplay+"\n"))
f.close()
